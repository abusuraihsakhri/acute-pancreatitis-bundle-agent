import pytest
from pancreas_guard import AtlantaSeverityClassifierAgent, SIRS24hTrajectoryAgent, HydrationFluidTitratorAgent, PancreasCoordinator, main


def test_sub_agents():
    a1 = AtlantaSeverityClassifierAgent()
    alerts1 = a1.evaluate({"metric_primary": 35.0})
    assert len(alerts1) == 1

    a2 = SIRS24hTrajectoryAgent()
    alerts2 = a2.evaluate({"critical_flag": True})
    assert len(alerts2) == 1

    a3 = HydrationFluidTitratorAgent()
    alerts3 = a3.evaluate({"status_text": "DISCORDANT_FINDING"})
    assert len(alerts3) == 1


def test_coordinator():
    coord = PancreasCoordinator()
...
def test_domain_registry():
    from pancreas_guard import DomainKnowledgeRegistry
